      ...
      <div className="flex flex-col gap-3">
        {items.map((item, idx) => (
          ...
          >
            ...
            {item.listHtml ? (
              <ul
                className="mt-0 pl-5 text-[var(--muted)] space-y-2"
                dangerouslySetInnerHTML={{ __html: item.listHtml }}
              />
            ) : null}
            {item.desc ? (
              <p className="text-[var(--muted)] mt-2 mb-0 leading-relaxed">{item.desc}</p>
            ) : null}
          </article>
        ))}
      </div>
