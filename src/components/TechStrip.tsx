...
const swaggerSvg = (
  <svg className="w-7 h-7 block" viewBox="0 0 256 256" role="img" aria-label="Swagger">
...
function renderChip(label: string, icon?: string, withSvg?: boolean, key?: string | number) {
  const baseClass =
    'inline-flex items-center justify-center w-11 h-11 border rounded-[14px] bg-[rgba(77,212,182,0.12)] text-[13px] font-semibold leading-tight min-w-0';
  ...
  if (withSvg) {
    return (
      <div
        ...
        title={label}
        style={{
          borderColor: 'rgba(77, 212, 182, 0.25)',
  ...
  const src = icon ? `/skill-icons/${icon}.svg` : undefined;
  ...
  return (
    <div
      ...
      key={key ?? `${label}-${icon}`}
    >
      {icon ? <img className="w-7 h-7 block" src={src} alt={label} loading="lazy" /> : null}
    </div>
  );
}

function MarqueeRow({ items, reverse }: { items: TechItem[]; reverse?: boolean }) {
  const duration = reverse ? '32s' : '24s';
  const direction = reverse ? 'reverse' : 'normal';
  const sequence = items.concat(items, items);

  return (
    <div className="flex flex-nowrap overflow-hidden px-3">
      <div
        className="flex flex-nowrap gap-2 py-2"
        style={{
          animation: `slide ${duration} linear infinite ${direction}`,
          width: 'max-content',
        }}
      >
        {sequence.map((item, idx) =>
          item.svg === 'swagger'
            ? renderChip(item.label, undefined, true, `${item.label}-${idx}`)
            : renderChip(item.label, item.icon, false, `${item.label}-${idx}`)
        )}
      </div>
    </div>
  );
...
export function TechStrip() {
  return (
    <div
      className="relative overflow-hidden border rounded-xl mb-5 py-3"
