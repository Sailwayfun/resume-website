...
function renderChip(label: string, icon?: string, withSvg?: boolean, key?: string | number) {
  ...
  return (
    <div
      ...
      key={key ?? `${label}-${icon}`}>
      {icon ? (
        <img className="w-7 h-7 block" src={`https://skillicons.dev/icons?i=${icon}`} alt={label} loading="lazy" />
      ) : null}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-nowrap overflow-hidden px-3">
      <div
        ...
        style={{ animation: `slide ${duration} linear infinite ${direction}`, width: 'max-content' }}>
